...
alter table public.tasks
  drop constraint if exists tasks_design_stage_check;

alter table public.tasks
  add constraint tasks_design_stage_check
  check (design_stage in ('concept','schematic','detailed','tender'));

create index if not exists tasks_design_stage_scope_idx
  on public.tasks(project_id,design_stage,status);
...
as $function$
  with scoped as materialized (
    ...
    select t.id,t.task_name,t.discipline_id,t.owner,t.status,t.priority,t.progress_stage,t.design_stage,t.start_date,t.due_date,
